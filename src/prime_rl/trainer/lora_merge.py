...
_LORA_KEY = re.compile(r"^(?P<prefix>.+)\.(?:(?P<projection>w1|w2|w3|gate_up|down)_)?lora_(?P<factor>[AB])\.0$")
    ...
    *,
    ...
    for base_key, (ndim, transposed, selected) in factors.items():
        ...
        if lora_a.dtype != base.dtype or lora_b.dtype != base.dtype:
            raise ValueError(
                f"incompatible LoRA dtypes for {base_key!r}: base={base.dtype}, A={lora_a.dtype}, B={lora_b.dtype}"
            )
        if lora_a.device != base.device or lora_b.device != base.device:
            raise ValueError(
                f"incompatible LoRA devices for {base_key!r}: base={base.device}, A={lora_a.device}, B={lora_b.device}"
            )
